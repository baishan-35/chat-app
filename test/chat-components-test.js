...
wsA.on('message', function message(data) {
  ...
  if (msg.type === 'chat_message') {
    messageCountA++;
    // 收到用户B的回复后发送确认消息
    if (msg.data?.content === '收到你的消息了') {
      setTimeout(() => {
        console.log('📤 用户A发送确认消息: "谢谢回复"');
        wsA.send(JSON.stringify({
          type: 'chat_message',
          data: {
            content: '谢谢回复',
...
wsB.on('message', function message(data) {
  ...
  if (msg.type === 'chat_message') {
    messageCountB++;
    // 如果收到用户A的消息，则回复
    if (msg.data?.content === 'Hello, 这是测试消息') {
      setTimeout(() => {
...
setTimeout(() => {
  console.log('\n📊 测试结果统计:');
  console.log(`用户A收发消息数: ${messageCountA}`);
  console.log(`用户B收发消息数: ${messageCountB}`);
  
  if (messageCountA >= 3 && messageCountB >= 3) {
    console.log('✅ 消息收发功能测试通过');
  } else {
    console.log('❌ 消息收发功能测试未通过');
  }
